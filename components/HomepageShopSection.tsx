...
// Maps lowercase collection names as stored in DB → URL slug
...
// Italian meaning + one-line description for each collection
...
async function fetchCollectionsForCategory(category: string): Promise<CollectionCard[]> {
  try {
...
export default async function HomepageShopSection({
  id,
  category,
  eyebrow,
  title,
  ...
  const collections = await fetchCollectionsForCategory(category);
